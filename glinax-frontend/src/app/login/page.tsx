...
export default function LoginPage() {
    ...
    const finalizeLogin = async (access: string, refresh: string, username: string) => {
        localStorage.setItem("access", access);
        localStorage.setItem("refresh", refresh);
        login(username);

        try {
            const { data: chats, error: chatError } = await getChatHistory();
            if (!chatError && chats.length > 0) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await finalizeLogin(access, refresh, user.username);
        } catch (err: any) {
                ...
                <div className="w-full max-w-md space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-3 max-w-[320px] mx-auto">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 py-2.5 border border-border rounded-xl hover:bg-secondary/50 transition-all text-sm disabled:opacity-60 disabled:cursor-not-allowed"
                        >
